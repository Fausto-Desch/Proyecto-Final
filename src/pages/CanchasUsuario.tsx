...
// Interfaz para el objeto de filtro
...
export default function CanchasUsuario() {
    ...
    const [filtroSlug, setFiltroSlug] = useState<string>("todos");
    ...
    const opcionesDeportes = useMemo(() => {
        const opciones: DeporteOption[] = [{ label: "Todos", slug: "todos" }];
        if (!club?.canchas) return opciones;

        const slugsExistentes = new Set<string>();

        club.canchas.forEach(cancha => {
            if (cancha.deporte) {
                const slug = cancha.deporte.toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
                if (!slugsExistentes.has(slug)) {
                    slugsExistentes.add(slug);
                    opciones.push({ label: cancha.deporte.trim(), slug });
                }
            }
        });
        return opciones;
    ...
    const canchasFiltradas = useMemo(() => {
        ...
        if (filtroSlug === "todos") return club.canchas;

        return club.canchas.filter(cancha => {
            const canchaSlug = cancha.deporte.toLowerCase().trim().normalize("NFD").replace(/[\u0300-\u036f]/g, "");
            return canchaSlug === filtroSlug;
        });
    }, [club?.canchas, filtroSlug]);
    ...
    if (loading) return (
        <div className="min-h-screen bg-slate-50 dark:bg-[#05080f] flex flex-col items-center justify-center gap-4">
            ...
            <p className="text-slate-400 font-medium italic">Sincronizando canchas...</p>
        </div>
    );

    if (error) return (
        <div className="min-h-screen flex flex-col items-center justify-center p-6 text-center bg-slate-50 dark:bg-[#05080f]">
            <AlertCircle className="text-red-500 mb-4" size={56} />
            <Link to="/clubes-usuario" className="bg-blue-600 text-white px-8 py-3 rounded-2xl font-bold">Volver</Link>
        </div>
    );
        ...
        <div className="flex flex-col min-h-screen bg-slate-50 dark:bg-[#05080f] text-slate-900 dark:text-white transition-colors duration-300">
            ...
            <div className="flex flex-1 overflow-hidden">
                ...
                <aside className="w-64 border-r border-slate-200 dark:border-gray-800/40 p-6 hidden lg:flex flex-col gap-4 bg-white dark:bg-[#05080f]">
                    ...
                    <SidebarInfo icon={<Trophy size={18} className="text-blue-500"/>} label="Deportes" value={opcionesDeportes.length > 1 ? (opcionesDeportes.length - 1).toString() : "0"} />
                ...
                <main className="flex-1 overflow-y-auto p-6 lg:p-12">
                        ...
                        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
                            ...
                            <div className="flex items-center gap-3">
                                ...
                                <div className="flex bg-white dark:bg-[#0d121f] p-1 rounded-xl border border-slate-200 dark:border-gray-800">
                                    {opcionesDeportes.map((opcion) => (
                                        <button
                                            key={opcion.slug}
                                            onClick={() => setFiltroSlug(opcion.slug)}
                                            className={`px-4 py-1.5 rounded-lg text-[10px] font-black uppercase transition-all ${
                                                filtroSlug === opcion.slug
                    ...
                    <motion.div layout className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                        <AnimatePresence mode="popLayout">
                            {canchasFiltradas.length === 0 ? (
                                <motion.div key="empty" initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="col-span-full text-center py-20 border-2 border-dashed border-slate-200 dark:border-gray-800 rounded-[2.5rem]">
                                    <p className="text-slate-400 font-medium">No hay canchas disponibles para este deporte.</p>
                                </motion.div>
                            ) : (
                                        ...
                                        <div className="p-7">
                                            ...
                                            <button
                                                ...
                                                className={`w-full text-[11px] font-black py-4 rounded-2xl flex items-center justify-center gap-2 transition-all
                                                    ${cancha.turno ? "bg-blue-600 hover:bg-blue-500 text-white shadow-lg shadow-blue-600/20" : "bg-slate-300 dark:bg-gray-700 text-slate-500 cursor-not-allowed"}
                                                `}
                                            >
                                                <Clock size={16} /> VER DISPONIBILIDAD
                                            </button>
                                        </div>
                                    </motion.div>
                                ))
                    ...
                    <div className="mt-16 flex justify-center pb-10">
                        <Link to="/clubes-usuario" className="flex items-center gap-3 bg-white dark:bg-[#0d121f] border border-slate-200 dark:border-gray-800 px-10 py-4 rounded-2xl text-[11px] font-bold text-slate-500 dark:text-gray-400 hover:text-blue-600 dark:hover:text-white transition-all shadow-sm group">
                            <ArrowLeft size={16} className="group-hover:-translate-x-1 transition-transform" /> VOLVER A LA SELECCIÓN DE CLUBES
                        </Link>
                    </div>
                </main>
            </div>
...
function SidebarInfo({ icon, label, value }: SidebarInfoProps) {
    return (
        <motion.div whileHover={{ x: 5 }} className="bg-white dark:bg-[#0d121f] p-4 rounded-2xl border border-slate-200 dark:border-gray-800/40 flex items-center justify-between shadow-sm dark:shadow-none">
